#Variational M Step - profile regression

maxStepProf <- function(X, y, model, prior){
  ...
  beta1 <- c()
  for(k in 1:K){
    beta0[k] <- priorbeta0 + sum(y * rnk[,k])
    beta1[k] <- priorbeta1 + sum((1-y) * rnk[,k])
  }
  ...
  model$beta0 <- beta0 #update beta0* in model
  model$beta1 <- beta1 #update beta1* in model
  return(model)
}
